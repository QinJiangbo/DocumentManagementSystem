...
<div class="content">
	...
	<div class="position">
		<span>当前位置：</span><span><a href="UserServlet?act=index">首页</a></span>&gt;<span><a
			href="ActionServlet?act=Undo&op=contract_technology_protocol">合同及技术协议审核</a></span>
	</div>
				...
				<tbody>
					<c:forEach items="${contractTechnologyProtocolRows}" var="row">
						<tr id="${row['项目编号']}">
							...
							<td class="col4"><a onclick="javascript:urlEncode2('ContractTechnologyProtocolServlet?op=verify&ch=${row['编号']}&method=init&projectType=合同及技术协议&projectId=${row['项目编号']}')" target="_blank">审核</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
