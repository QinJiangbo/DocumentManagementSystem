...
<div class="content">
	...
	<div class="position">
		<span>当前位置：</span><span><a href="UserServlet?act=index">首页</a></span>&gt;<span><a
			href="ActionServlet?act=Undo&op=project_expert">工程专家信息审核</a></span>
	</div>
				...
				<tbody>
					<c:forEach items="${projectExpertRows}" var="row">
						<tr id="${row['姓名']}">
							...
							<td class="col4"><a onclick="javascript:urlEncode2('ProjectExpertServlet?op=verify&ch=${row['编号']}&method=init&projectType=工程专家&projectId=${row['身份证']}')" target="_blank">审核</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
